...
- **环境关联账号抽屉**（`src/renderer/src/pages/Environments.tsx`，提交 `419cd64`）：环境列表每行新增「账号」按钮，打开抽屉调用 `GET /api/accounts?profileId=<环境ID>`（该参数在前一轮账号列表检索已落地），列出该环境名下的全部账号（平台/账号/备注）。后端零改动，纯前端闭环「账号-环境批量关联」能力，让归属可见可查。
- 抽屉只读展示，账号的编辑/导入/批量关联仍在账号中心进行。
...
- **收藏（新增）**（`src/renderer/src/pages/Environments.tsx`）：环境列表首列星标按钮一键收藏；工具栏「仅看收藏」开关（带收藏计数）。收藏集存于浏览器 localStorage（key `roxy_starred_envs`），不落库、不加数据库列（符合规则 #24）；删除/回收环境后残留 ID 随列表刷新自动裁剪。列表数据源经 `viewList` 派生，与既有后端 keyword/group 过滤叠加，三层筛选互不冲突。
- **搜索 / 筛选（既有，本次补齐文档）**：顶部关键词搜索（名称/备注/平台，`GET /api/profiles?keyword=`）与分组下拉筛选（`?groupId=`）早已存在且可叠加，本次在文档中正式纳入「列表检索」能力说明。
- 提交：`f3a5ceb`（feat）。
...
- **智能代理分配**（`src/shared/proxyAllocator.ts` 纯函数 + `src/main/server.ts` 路由 `POST /api/profiles/batch-allocate-proxy` + `src/renderer/src/pages/Environments.tsx` 弹窗）：环境列表多选后一键「智能分配」，从可用代理池逐一分配**互不相同**的出口 IP——彻底规避「两个环境共用同一代理」这一防关联大忌（与旧的「绑定代理 = 全部绑同一代理」反模式相反）。支持按国家 / 类型过滤；前端弹窗两阶段（筛选 → 结果，四张统计卡 + 结果明细表）。
- 分配核心逻辑抽成无依赖纯函数 `allocateProxies`（两阶段：Phase 1 锁定已独占合规代理的环境、Phase 2 从可用池取互不相同的代理），调用方只负责取数 / 落库 / 写日志。离线单测 20 例全绿（互不相同 / 池不足跳过 / 保留独占 / 运行中跳过 / 共享代理释放重分配 / 国家过滤 / 失效过期排除 / updates 与 assignedProxyIds 一致）。
- 不新增数据库列（只改已有 `ProfileEntity.proxyId`，符合规则 #24）。

### 设计要点

- 可用池 = 未失效、未过期、符合过滤、且**未被未选中环境占用**的代理——既含全局空闲代理，也含「仅被本批选中环境共享」的代理（后者可重新平衡，释放后打散到各选中环境各自独占出口，是防关联核心收益）。
- 池不足时宁可 `skipped` 也绝不把某代理分给第二个环境；运行中环境不改动绑定。
- 提交：`82207ba`（feat）。

---

## 2026-09-26 · 批量体检总览（应用层）

### 新增

- **批量体检总览**（`src/main/server.ts` + `src/renderer/src/pages/Environments.tsx`）：环境列表多选后一键「批量体检」，新增 `POST /api/profiles/batch-healthcheck` 端点——逐个 `running` 环境复用单环境体检全链路（`probeFingerprint` + `buildHealthReport`，含 `audioSeed` 同源与代理出口国家），未运行环境标记为 `skipped`（不自动开关窗口），探针并发受限（默认 10）。返回聚合结果：平均伪装度 / 通过数 / **系统级问题分布**（各检查项失败环境数，按失败降序，快速暴露共性配置错误）/ 逐环境完整报告。
- 前端「批量体检总览」抽屉：四个统计卡 + 系统级问题分布进度条 + 逐环境表格（可展开看逐项 expected/actual/ok 明细），复用单环境体检表格与 `HEALTH_LABELS`。批量工具栏新增「批量体检」按钮。
- 不新增数据库列（纯聚合计算，符合规则 #24）。

### 设计要点

- 仅对 `running` 环境采集（探针必须在真实窗口内执行，与单环境体检一致）；未运行标记跳过而非自动打开，避免改动用户状态。
- 探针并发受限，避免同时几十个窗口 JS 采集打爆主进程。
- 提交：`5af4187`（feat）。
