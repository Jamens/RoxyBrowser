...
- **列表搜索 / 筛选 / 收藏（应用层）**：环境列表支持关键词搜索（名称/备注/平台）+ 分组筛选（两者可叠加，后端 GET /api/profiles 参数）；新增首列星标「收藏」与工具栏「仅看收藏」开关，收藏集存于浏览器 localStorage（不落库、不加列），删除环境后自动裁剪脏 ID。；仅对当前用户可见的账号生效（member 只动自己账号），目标环境须为有效非模板环境。免去逐条在编辑弹窗里选环境。——彻底规避「两个环境共用同一代理」这一防关联大忌（与旧的「绑定代理=全部绑同一代理」反模式相反）。支持按国家 / 类型过滤；已独占合规代理的环境原地保留、运行中的环境不动、池不足的环境标记跳过（绝不强行复用）。分配核心逻辑抽成纯函数 allocateProxies（已离线单测 20 例全绿）。
