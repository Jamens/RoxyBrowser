...
- **账号-环境批量关联（应用层）**：账号中心列表勾选多个账号，一键统一绑定到某个目标环境（改写 AccountEntity.profileId）；仅对当前用户可见的账号生效（member 只动自己账号），目标环境须为有效非模板环境。免去逐条在编辑弹窗里选环境。——彻底规避「两个环境共用同一代理」这一防关联大忌（与旧的「绑定代理=全部绑同一代理」反模式相反）。支持按国家 / 类型过滤；已独占合规代理的环境原地保留、运行中的环境不动、池不足的环境标记跳过（绝不强行复用）。分配核心逻辑抽成纯函数 allocateProxies（已离线单测 20 例全绿）。
