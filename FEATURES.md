...
- **指纹预设库**：内置十余套「验证过的指纹组合」（如 `Windows 11 · Chrome 154 · 德国`、`Pixel 8 · Android 14 · 美东`、`iPhone 15 Pro · iOS 17.5 · 美西`），各字段之间保证一致（UA ↔ 平台 ↔ GPU ↔ 屏幕 ↔ 时区语言），一键套用，避免手工拼出互相矛盾的指纹。
- **内核版本切换（Chrome 大版本）**：指纹表单「内核版本 (Chrome)」下拉可选 **150–154** 任一版本——仅替换 UA 串里的 Chrome 大版本号（如 `Chrome/129.0.6668.100`），同步更新 `uaFullVersion` 与 UA-CH 客户端提示（`navigator.userAgentData`），让同一套设备指纹在不同时期表现为不同浏览器版本（对标官方「内核版本」切换）。iOS 走 Safari/WebKit，不走此路径（下拉自动隐藏）。
...
环境体检是「自己出题自己判卷」——拿设定值对撞窗口内回读值，只能证明注入生效；平台真正识别账号靠的是**外部检测站**，它们从真实人群分布的角度判断「这几个指标组合起来像不像一个人」。两者互补，缺一不可。

#### 流程

1. 前端在体检报告 Drawer 内列出 4 个内置检测站；点某个站点 → `POST /api/profiles/:id/scan { siteId }`。
2. 后端 `browserManager.scanSite` 把环境窗口导航到该站点（这是有意的，结果页本身就是证据），等待主帧加载 + SPA 异步渲染，然后 `executeJavaScript` 抓页面可见文本 + `capturePage` 截图。
3. 结果回写：页面标题、实际落地 URL、去噪文本、按站点 `hints` 摘录的关键行、截图 data URL、抓取时间戳。

#### 内置站点（按维度递进）

| 站点 | 看什么 | 等待 |
| --- | --- | --- |
| PixelScan | 一致性：代理出口 IP ↔ 时区 ↔ 浏览器语言 是否自洽（最贴近防关联诉求） | 9s |
| BrowserLeaks | 综合指纹：Canvas / WebGL / 字体 / 媒体设备 / WebRTC 各项实测 | 5s |
| CreepJS | 深度分析：trust score、是否被识破（lies）、各项熵值 | 12s |
| AmIUnique | 唯一性：你的指纹在多大规模样本里是唯一的（越唯一越易被追踪） | 7s |

#### 设计取舍

- **证据优先、判定从简**：检测站 DOM 常改版，依赖精确选择器的解析会腐坏，所以只抓通用 `innerText` + 截图，**不下绝对的通过 / 不通过结论**——避免误判，也让站点改版不至于让整个功能失效。
- **并发安全**：同一环境同时只允许一次检测（in-flight 锁）。叠加两次导航会让前一次读到后一个站点的页面，造成「站点名是 A、内容是 B」的张冠李戴——对本功能（卖点就是证据）是致命的，故在 `browserManager.scanSite` 直接拦截为「该环境正在检测中」。
- **失败也留证**：`describeLoadError` 把 Chromium 网络错误码（ERR_PROXY_CONNECTION_FAILED / ERR_NAME_NOT_RESOLVED / 证书 / 超时等）翻译成中文；网络不通 / 代理失效 / 撞上人机验证（Cloudflare）时，仍尽力带上截图与错误页文本，并把异常写入操作日志（这类记录恰恰最需要追溯）。
- **主帧判定**：等待加载完成必须判 `isMainFrame`——iframe / 子资源失败很常见，不能让它提前放行（否则 SPA 还没渲染就抓文本）。
- **不自动跳回**：用完停在检测站结果页，跳回会重新加载、可能丢失用户登录态或表单；需要时请自行后退。
- **隐私提示**：UI 用 Alert 明确告知「会把环境窗口真实指纹发送给第三方站点」，对高价值账号环境需谨慎。

#### 纯函数可单测

- `extractHighlights(text, hints, max)`：按关键词从正文摘录相关行（每词最多 2 行、总数封顶、行级去重、单行 ≥300 字符视为段落跳过），离线单测覆盖命中 / 去重 / 上限 / 空入参。

### 7.14 批量体检总览（应用层）

单环境体检只能「一个一个看」，而跨境电商用户往往同时管几十上百个环境——要快速知道「这批环境整体伪装度如何」「有没有共性配置错误」，单环境视图无能为力。批量体检总览把前面所有反检测体检项（webdriver / 平台 API / 媒体查询 / 字体 OS 一致性 / WebGPU / EME …）从「逐环境」升维到「批量聚合」。

- **入口**：环境列表多选（已有 `rowSelection`）→ 批量工具栏「批量体检」按钮 → `POST /api/profiles/batch-healthcheck { ids }`。
- **后端聚合**：对每个 `running` 环境复用单环境体检全链路（`probeFingerprint` + `buildHealthReport`，含 `audioSeed` 同源与代理出口国家），未运行环境标记为 `skipped`（**不自动开关窗口**，避免改动用户状态）；探针**并发受限（默认 10）**，避免同时几十个窗口 JS 采集把主进程打爆。返回结构含：
  - `summary`：`total / probed / skipped / avgScore / passCount`（通过 = 伪装度 ≥ 80）/ `passThreshold`；
  - `itemStats`：每个体检项的 `passed / failed / na` 计数，按 `failed` 降序——这就是「系统级问题分布」；
  - `profiles`：逐环境完整 `HealthReport`（伪装度、逐项 expected/actual/ok），供前端展开明细。
- **前端总览抽屉**：顶部四个统计卡（平均伪装度 / 通过数 / 已探测·跳过 / 选中总数）；其下「系统级问题分布」用进度条列出失败环境数最多的检查项（如「字体与 OS 一致性：12 环境」一眼看出共性错误）；再下「逐环境报告」表格，每行可展开看该环境的逐项对撞明细（复用单环境体检的表格与 `HEALTH_LABELS`）。
- **不新增数据库列**：纯聚合计算，无新字段、无新迁移（符合规则 #24）。
- **典型工作流**：批量打开 → 批量体检 → 从系统级问题分布定位共性错误 → 批量改代理 / 重随机指纹 / 批量移动分组修正 → 再体检复核。

### 7.15 智能代理分配（负载均衡 + 避免同代理复用，应用层）

批量管理时，给每个环境逐一手挑代理既慢又容易「漏网」——把两个环境绑到同一出口 IP 是防关联大忌。已有的「绑定代理」（`bindProxy`）是把**全部选中环境绑到同一个代理**，本质是与防关联目标相悖的反模式。智能代理分配则以「**每个环境拿到的代理互不相同**」为核心约束，从可用池逐一分配，把负载摊平、把复用清零。

- **入口**：环境列表多选 → 批量工具栏「智能分配」按钮（`DeploymentUnitOutlined`）→ 弹窗两阶段（筛选 → 结果）。
  - 筛选阶段：可选「国家 / 类型」过滤（如只从 US 住宅代理里分配）；点「开始分配」调 `POST /api/profiles/batch-allocate-proxy { ids, country?, type? }`。
  - 结果阶段：四张统计卡（已分配 / 已保留 / 运行中跳过 / 池不足跳过）+ 结果明细表（每行显示环境、动作 `assigned|kept|skipped`、分配到的代理 ID、跳过原因）；若池不足会高亮告警。
- **后端纯函数 `allocateProxies`**（`src/shared/proxyAllocator.ts`，无 DB / Electron 依赖，已离线单测 20 例全绿）：调用方（`server.ts` 路由）只负责取数、落库（`profileRepo.save` 只改 `proxyId`）、写操作日志，分配不变量全部在纯函数里沉淀，便于无依赖单测。核心规则：
  1. **可用池 = 未失效、未过期、符合 country/type 过滤，且未被「未选中环境」占用**（`outsiderUsage===0`）的代理。它既含全局空闲代理，也含「仅被本批选中环境共享」的代理——后者是本批**可重新平衡**的，必须释放并打散到各选中环境，让原本挤在同一代理上的多个环境各自独占一个出口（防关联核心收益）。
  2. **保留独占**：Phase 1 先锁定「已独占一个合规代理（仅自己用、无外人）」的环境，避免被后面的空环境从池里抢走，最小改动、不制造无谓切换。
  3. **互不相同的分配**：Phase 2 处理剩余环境（无代理、或代理被共享 / 被外人占用），从可用池按 id 顺序取一个未被本轮占用的代理；**池不够时该环境标记 `skipped`（不强行复用，避免关联）**。
  4. **运行中环境不改动绑定**，标记 `skipped`（与批量体检一致，不自动开关窗口）。
- **与 `bindProxy` 对比**：`bindProxy` 是「全部绑同一代理」（反关联反模式）；智能分配是「互不相同」（防关联正解）。故池不足时宁可 `skipped` 也绝不把某个代理分给第二个环境。
...
- **闭环（环境侧查看关联账号）**：环境列表每行新增「账号」按钮（UserOutlined），打开抽屉调用 `GET /api/accounts?profileId=<环境ID>`（该参数在「列表搜索/筛选/收藏」一轮已落地），直接列出该环境名下的全部账号（平台 / 账号 / 备注），让「账号→环境」的归属**可见可查**——否则批量关联后用户还得去账号中心反查，体验断裂。抽屉只读展示，编辑/导入仍在账号中心。

### 7.17 列表搜索 / 筛选 / 收藏（应用层，环境列表 + 账号列表）

跨境电商用户的环境/账号列表往往有几十上百条，要快速定位「某个店铺」「某分组」「常驻关注的那几个」必须靠列表检索能力。该能力已**同时覆盖「环境列表」与「账号列表」**两套主列表（账号列表此前完全没有检索，本次补齐到与环境列表同构）。

**环境列表（既有 + 收藏补全，见提交 `f3a5ceb`）**：
- **搜索（关键词）**：顶部搜索框（名称 / 备注 / 平台 LIKE），随输入实时回传 `GET /api/profiles?keyword=`，后端 `qb.andWhere('(p.name LIKE :kw OR p.remark LIKE :kw OR p.platform LIKE :kw)')` 命中；`allowClear` 一键清空。
- **筛选（分组）**：分组下拉（`GET /api/profiles?groupId=` → `qb.andWhere('p.groupId = :gid')`），与搜索可叠加；「全部分组」即不过滤。
- **收藏**：列表首列星标按钮，一键把环境加入「我的收藏」；工具栏「仅看收藏」按钮切换只看收藏（带当前收藏计数）。收藏集存于**浏览器 localStorage**（key `roxy_starred_envs`），**不落库、不加数据库列**（符合规则 #24）——收藏是个人隐私偏好，无需进团队库；换机器不共享属可接受取舍。实现要点：
...
**账号列表（本次扩展，提交 `b4a10f4`）**：原本无任何检索，现与环境列表同构补齐：
- **搜索（关键词）**：`GET /api/accounts?keyword=` 后端在「团队非模板环境归属」过滤之上，再按「平台 + 账号 + 备注」拼接串做大小写不敏感包含匹配；`allowClear` 清空。
- **筛选**：① 所属环境下拉（`?profileId=`，即「只看某环境的账号」）；② 平台下拉（`?platform=`，复用 `PLATFORMS`）。两者与关键词可叠加。
- **收藏**：列表首列星标按钮 + 工具栏「仅看收藏」开关，收藏集存 **localStorage**（key `roxy_starred_accounts`），实现与裁剪逻辑同环境列表；列表数据源经 `viewList` 派生，与后端 keyword/环境/平台筛选正交叠加。
- **前端 `load` 改为带 query 参数的 `useCallback`**，依赖 `[keyword, profileFilter, platformFilter]`，筛选变更即重拉；保存/导入/批量关联后回拉时保留当前筛选（不丢上下文）。

- **不新增数据库列**：两套列表的收藏均用 localStorage，搜索/筛选走既有/新增的 `GET` 参数（规则 #24）。
